...
First_half                       = bool(int(sys.argv[3]))
#First_half                       = True #True: Will run the first half of the possible hidden sizes. Set to False for the second half
ensure_directory_for_file(sys.argv[4])
# In[ ]:


if First_half:
    hidden_dimension_sizes = list(range(1,transformation_config["in_features"]//2+1))
else:
    hidden_dimension_sizes = list(range(transformation_config["in_features"]//2+1,transformation_config["in_features"]+1))
